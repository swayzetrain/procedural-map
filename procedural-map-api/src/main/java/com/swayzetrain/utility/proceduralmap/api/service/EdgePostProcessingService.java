	...
	public void postProcessMapEdges(MapDataPoint[][] mapData) {
		...
		for (int y = 0; y < mapData.length; y++) {
			for (int x = 0; x < mapData[y].length; x++) {
				...
				switch (middleMiddle) {
					case PATH:
						mapData[y][x].setPostProcessedTileType(PostProcessedTileType.PATH);
						break;
					case WALL:
							PostProcessedTileType postProcessedTileType = postProcessingDecisionTreeComponent
									.getPostProcessingDecisionTreeCollection().stream()
